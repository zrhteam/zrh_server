...
def region_distribution():
    ...
    for item in cache_cascade_record:
        if item.project_code in contained_check_map.keys():
            if contained_check_map[item.project_code]["project_tag"] not in resp_data["data"]["project_distribution"].keys():
                resp_data["data"]["project_distribution"][contained_check_map[item.project_code]["project_tag"]] = {"risk_level": {"1": 0, "2": 0, "3": 0},
                                                                               "major": {}}
            resp_data["data"]["project_distribution"][contained_check_map[item.project_code]["project_tag"]]["risk_level"][str(item.risk_level)] += 1
            if item.major_name not in resp_data["data"]["project_distribution"][contained_check_map[item.project_code]["project_tag"]]["major"].keys():
                resp_data["data"]["project_distribution"][contained_check_map[item.project_code]["project_tag"]]["major"][item.major_name] = 0
            resp_data["data"]["project_distribution"][contained_check_map[item.project_code]["project_tag"]]["major"][item.major_name] += 1
    print("Returned data: ")
    print(resp_data)
    end_t = datetime.now()
    print("Query total time is: " + str((end_t - start_t).seconds) + "s")
...
def region_index_rank():
    ...
    for item in cache_cascade_record:
        if item.project_code in contained_check_map.keys():
            ...
            if str(item.risk_level) == "3":
                risk_project_map[contained_check_map[item.project_code]["project_tag"]] += 1
    res = sorted(risk_project_map.items(), key=lambda d: d[1], reverse=True)
    idx = 0
    for ele in res:
        resp_data["data"][ele[0]] = {"rank": idx, "count": ele[1]}
